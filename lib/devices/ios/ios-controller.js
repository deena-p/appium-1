...
iOSController.findUIElementOrElements = function(strategy, selector, ctx, many, cb) {
  var unescapedSelector = selector;
  ...
  var doFind = function(findCb) {
    ...
    if (strategy === "name") {
      ...
      command = ["au.getElementsByPath('", selector, "'", ctx, ")"].join('');
  ...
  if (strategy === "xpath") {
    this.proxy("wd_frame.getPageSource()", function(err, res) {
      var helper = function(xmlNode, json) {
        xmlNode.set('name', json.name);
        xmlNode.set('label', json.label);
        xmlNode.set('value', json.value);
        xmlNode.set('x', json.rect.origin.x);
        xmlNode.set('y', json.rect.origin.y);
        xmlNode.set('width', json.rect.size.width);
        xmlNode.set('height', json.rect.size.height);
        xmlNode.set('enabled', json.enabled);
        xmlNode.set('valid', json.valid);
        xmlNode.set('visible', json.visible);
        xmlNode.set('path', json.path);

        var numChildren = json.children.length;
        for ( var i = 0; i < numChildren; i++) {
          var jsonChild = json.children[i];
          var elementType = /*typeShortcuts[*/jsonChild.type/*]*/;
          var xmlChild = et.SubElement(xmlNode, elementType !== null ? elementType : jsonChild.type);
          helper(xmlChild, jsonChild);
        }
      };
      var source = JSON.parse(res.value);
      var root = et.Element(source.type);
      helper(root, source);
      var etree = new et.ElementTree(root);
      var xmlText = etree.write({'xml_declaration': false});
      var doc = new dom().parseFromString(xmlText);
      var xmlResults = xpath.select(unescapedSelector, doc);
      var results = '';
      for (var i=0; i < xmlResults.length; i++) {
        results += results.length > 0 ? ',' : '';
        results += (xpath.select1(xmlResults[i], "/@path"));
      }
      selector = results;
      this.waitForCondition(this.implicitWaitMs, doFind, cb);
    });
  } else if (_.contains(this.supportedStrategies, strategy)) {
    this.waitForCondition(this.implicitWaitMs, doFind, cb);
  } else {
    cb(null, {
