...
var mechanic = (function() {
  ...
  UIAElement.prototype.getElementsByPath = function(givenPath) {
      var foundElsHelper = function(element, selector, acc) {
        var toFind = selector.split(',');
        for (var i=0; i < toFind.length; i++) {
          try {
            var path = toFind[i].split('/');
            var found = this.mainApp;
            for (var j=1; j < path.length; j++) {
              var index = parseInt(path[j]);
              found = found.elements()[index];
            }
            acc.push(found);
          } catch(err) {
      ...
      var foundEls = [];
      foundElsHelper(app, givenPath, foundEls);
      return foundEls;
    };
