...
UIAElement.prototype.getTree = function() {
  ...
  var getTree = function(element, elementPath) {
    var subtree = {
      name: element.name()
      , type: element.type()
      , label: element.label()
    ...
    au.pathsForLastTree[elementPath] = element;
    var children = element.elements();
    var numChildren = children.length;
    for ( var i = 0; i < numChildren; i++) {
      var child = children[i];
      subtree.children.push(getTree(child, elementPath + "/*[" + (i+1).toString() + "]"));
    }
    return subtree;
  };
  au.pathsForLastTree = new Object();
